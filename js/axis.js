axis = function(elemid, vector, XorY, options) {
  //changed top margin of axis from 30 to 45
  ...
  if (istxtdata==true) {data = data.slice(0, 15);}
  ...
  if (XorY == "X") {
    ...
    .enter().append("rect")
      ...
      .attr("width", function(d) { if (d["value"]==0) {return 4;} else {return Math.abs(x(d["value"]) - x(0));} })
  ...
  var dragBarTop = svg.selectAll(".dummy")
    ...
    .enter().append("rect")
      ...
      .call(d3.behavior.drag().on('drag', function(d,i) {
        ...
        ial.usermodel.setAttributeWeightVector(V, true, {'level': 'INFO', 'eventType': 'set_attribute_weight_vector_drag', 'whichAxis': XorY, 'userId': window.localStorage.getItem("userId")});
